...
const BUTTONS = [
  { id: 1, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 1 },
  { id: 2, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 2 },
  { id: 3, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 3 },
  { id: 4, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 4 },
  { id: 5, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 5 },
  { id: 6, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 6 },
  { id: 7, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 7 },
  { id: 8, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 8 },
  { id: 9, label: 'EDITAR CENTRO', subLabel: 'Puls+ 1 Seg', colorKey: 9 },
];
...
function CentroButton({ 
  ...
  return (
    <motion.button
      ...
      className={`${TESO_COLORS[btn.colorKey as keyof typeof TESO_COLORS]} aspect-square rounded-2xl flex flex-col items-start p-3 shadow-lg relative overflow-hidden group`}
    >
...
export default function Centros2Page({ centroNames, setCentroNames, onBack, onGoLanding, onSelectZonas, onOpenIA, onOpenConfiguracion }: Centros2PageProps) {
  ...
  React.useEffect(() => {
    setButtons(BUTTONS.map(b => ({
      ...b,
      label: centroNames[b.id + 7] || b.label
    })));
  }, [centroNames]);

  const handleEdit = (id: number, newLabel: string) => {
    setCentroNames(prev => ({ ...prev, [id + 7]: newLabel }));
      ...
      <div className={`w-full max-w-md flex flex-col ${isHostingVisible ? 'gap-4 sm:gap-6 my-auto' : 'gap-8 h-full justify-center'} shrink-0 py-6 transition-all duration-500`}>
        <header className={`flex justify-between items-center pt-2 transition-all duration-500 ${isHostingVisible ? 'opacity-100' : 'opacity-40 scale-90'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-2xl font-black tracking-tight text-white uppercase">CENTROS 2</p>
          </div>
        </header>
        ...
        <main className={`grid grid-cols-3 gap-3 sm:gap-4 transition-all duration-500 ${isHostingVisible ? '' : 'scale-110'}`}>
          {buttons.map((btn, index) => (
            <CentroButton
              ...
              onEdit={handleEdit}
              isBlackText={isBlackText(btn.colorKey)}
              onBack={onBack}
        ...
        >
          {buttons.map((btn, index) => (
            <motion.button
              key={`${btn.id}-${index}`}
              whileTap={{ scale: 0.9 }}
              onClick={() => onSelectZonas(btn.id + 9)}
